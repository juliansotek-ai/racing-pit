...
async function getHorseBets(userId: string, horseId: string) {
  return prisma.bet.findMany({
    where: { userId, raceEntry: { horseId } },
    include: {
      raceEntry: {
        include: {
...
type HorseBet = Awaited<ReturnType<typeof getHorseBets>>[number];
...
export default async function HorsePage({
  ...
  const [horse, session] = await Promise.all([getHorse(id), auth()]);
  if (!horse) notFound();

  const bets = session?.user?.id ? await getHorseBets(session.user.id, id) : [];
    ...
    <main className="flex flex-col min-h-screen px-6 py-12 max-w-5xl mx-auto gap-8">
                ...
                <GlassCard key={entry.id} variant="subtle" radius="lg" padding="md">
                  ...
                  <div className="sm:hidden flex flex-col gap-2">
                    <Link href={`/races/${entry.race.id}`} className="flex flex-col min-w-0 hover:underline">
                      ...
                      <span className="text-xs" style={{ color: "var(--text-tertiary)" }}>{entry.race.racecourse.name}</span>
                    </Link>
                    <div className="flex flex-wrap gap-x-2 gap-y-0.5 text-xs" style={{ color: "var(--text-secondary)" }}>
                      <span>{entry.jockey?.name ?? "No jockey"}</span>
                  ...
                  {/* Desktop */}
                  <div className="hidden sm:grid gap-4 items-center" style={{ gridTemplateColumns: "1fr 1fr 5rem 4rem 4rem" }}>
                    <Link href={`/races/${entry.race.id}`} className="flex flex-col min-w-0 hover:underline">
      ...
      {session?.user?.id && (
        <section className="flex flex-col gap-4">
          ...
          {bets.length === 0 ? (
            <GlassCard variant="subtle" radius="xl" padding="lg">
              <p style={{ color: "var(--text-secondary)" }}>No bets placed on this horse.</p>
            </GlassCard>
          ) : (
            <div className="flex flex-col gap-2">
              {bets.map((bet) => <HorseBetRow key={bet.id} bet={bet} />)}
            </div>
          )}
        </section>
      )}
...
function HorseBetRow({ bet }: { bet: HorseBet }) {
    ...
    <GlassCard variant={isWon ? "default" : "subtle"} radius="lg" padding="md" className="relative overflow-hidden">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex flex-col gap-0.5">
          <Link href={`/races/${entry.race.id}`} className="text-sm font-semibold hover:underline" style={{ color: "var(--text-primary)", fontFamily: "var(--font-display)" }}>
            {entry.race.name}
          </Link>
          <span className="text-xs" style={{ color: "var(--text-tertiary)" }}>
            {entry.race.racecourse.name} · {format(new Date(entry.race.scheduledAt), "d MMM yyyy")}
          </span>
        </div>
